      ...
      <div v-for="playlist in playlists" :key="playlist.id"
        ...
        <div class="aspect-w-1 aspect-h-1 w-full overflow-hidden relative cursor-pointer"
          ...
          <div
            ...
            <EyeIcon class="h-12 w-12 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
          </div>
        </div>
        <div class="p-4 flex-grow flex flex-col justify-between">
          <div>
            <h3 class="text-lg font-semibold text-gray-800 mb-2 line-clamp-1">{{ playlist.playlist_name }}
            </h3>
          ...
          <TransitionChild as="template" enter="ease-out duration-300"
            ...
            <DialogPanel
              ...
              <div
                ...
                <div class="grid w-full grid-cols-1 items-start gap-x-6 gap-y-8 sm:grid-cols-12 lg:gap-x-8 mt-8">
                  <div
                  ...
                  <div class="sm:col-span-8 lg:col-span-7">
                    <div class="flex items-center justify-between">
                      ...
                      <div class="flex items-center text-gray-600">
                        ...
                        <span class="text-lg font-semibold text-gray-800">{{
                          formatNumber(selectedPlaylist.like_count)
                        }}</span>
                      </div>
                    </div>
                    ...
                    <section aria-labelledby="information-heading" class="mt-4">
                      ...
                      <div class="flex flex-wrap gap-2 mb-4">
                        <span v-for="(tag, index) in selectedPlaylist.tags.split('-')" :key="index"
                    ...
                    <section aria-labelledby="description-heading" class="mt-6">
                      <h3 id="description-heading" class="text-lg font-medium text-gray-800 flex items-center">
                    ...
                    <div class="mt-8 flex space-x-4">
                      <button @click="handleLikePlaylist" :class="[
                        ...
                        <HeartIcon :class="['h-5 w-5 mr-2', isLiked ? 'text-white' : 'text-gray-400']" />
                        {{ isLiked ? '取消收藏' : '收藏歌单' }}
                      </button>
                  ...
                  <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
                    ...
                    <div class="mt-2">
                      <form @submit.prevent="submitPlaylist">
                        <div class="mb-4">
                          <label for="playlist-name" class="block text-sm font-medium text-gray-700">歌单名称</label>
                          <input type="text" id="playlist-name" v-model="newPlaylist.playlist_name" required
                        ...
                        <div class="mb-4">
                          ...
                          <textarea id="description" v-model="newPlaylist.description" rows="3"
...
import { PlaylistInfo } from '../../types/global.ts'
...
const checkLikeStatus = async () => {
  try {
    const result = await checkUserLikedPlaylist(userId, selectedPlaylist.value.id)
...
async function handleLikePlaylist() {
  try {
    if (isLiked.value) {
      await unlikePlaylist(userId, selectedPlaylist.value.id)
      isLiked.value = false
      selectedPlaylist.value.like_count--
    } else {
      await likePlaylist(userId, selectedPlaylist.value.id)
      isLiked.value = true
      selectedPlaylist.value.like_count++
    }
  } catch (error) {
